set.seed(1)
TSC_split= initial_split(TSC_Pasture, prop = 501/502, strata = logConc)
TSC_train = training(TSC_split)
TSC_test = testing(TSC_split)

TSC_recipe = recipe(logConc ~ ., data = TSC_Pasture) %>% 
...
TSC_fit = TSC_wf %>% fit(data = TSC_Pasture)
...
TSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = TSC_Pasture,
  y = TSC_Pasture$logConc,
  verbose = F
)


numVar = c("CowNum", "SPCvar", "CertYear", "FullEmpNum", "PplNumPerWk", "StockDen", "PplNumPerShift") #PC1
catVar = c("Loc", "PartEmp", "CowMilkLoc", "Parlor", "ClipFlame", "CornSilage", "ParlorManScrap")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_Pasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/TSC/NumVars_TSC_Pasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_Pasture_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/TSC/CatVars_TSC_Pasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")