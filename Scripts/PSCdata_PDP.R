set.seed(1)
PSC_split= initial_split(PSCdata, prop = 501/502, strata = Pres)
...
PSC_fit = PSC_wf %>% fit(data = PSCdata)
...
numVar = c("CowNum", "NonFamEmpNum", "PplNumPerWk")
catVar = c("HouseStyle", "TowelMacDry", "ParlorManScrap", "Parlor", "NonFamEmp", "CowMilkLoc", "TowelType", "ParlorHose", "Haylage",
           "CowHoldClean", "GloveFreq", "CornSilage")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/PSC/NumVars_PSC.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/PSC/CatVars_PSC.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")