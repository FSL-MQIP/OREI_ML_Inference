set.seed(1)
TSC_split= initial_split(TSC_newFarm, prop = 501/502, strata = logConc)
...
TSC_fit = TSC_wf %>% fit(data = TSC_newFarm)
...
TSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = TSC_newFarm,
  y = TSC_newFarm$logConc,
  verbose = F
)


numVar = c("CowNum", "CertYear", "FullEmpNum", "StockDen", "NonFamEmpNum", "PplNumPerWk", "PastureTime",
           "SPCvar")
catVar = c("ParlorHose", "Parlor", "NonFamEmp", "Glove", "CowMilkLoc", "GloveFreq", "CowWaitMilk")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_newFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/TSC/NumVars_TSC_newFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_newFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/TSC/CatVars_TSC_newFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")