set.seed(1)
PSC_split= initial_split(PSC_noParlor, prop = 501/502, strata = Pres)
PSC_train = training(PSC_split)
PSC_test = testing(PSC_split)

PSC_recipe = recipe(Pres ~ ., data = PSC_noParlor) %>% 
...
PSC_fit = PSC_wf %>% fit(data = PSC_noParlor)
...
numVar = c("PplNumPerWk", "CertYear", "StockDen", "SPCvar", "PplNumPerShift") #PC1
catVar = c("CornMeal", "Loc", "CornSilage", "ClipFlame", "CowParlorClean", "BioFeedAdd", "PreDipType", "Haylage", "GloveFreq")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_noParlor_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/PSC/NumVars_PSC_noParlor.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_noParlor_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/PSC/CatVars_PSC_noParlor.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")