set.seed(1)
MSC_split= initial_split(MSC_noPasture, prop = 501/502, strata = logConc)
MSC_train = training(MSC_split)
MSC_test = testing(MSC_split)

MSC_recipe = recipe(logConc ~ ., data = MSC_noPasture) %>% 
...
MSC_fit = MSC_wf %>% fit(data = MSC_noPasture)
...
MSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = MSC_noPasture,
  y = MSC_noPasture$logConc,
  verbose = F
)


numVar = c("CowNum", "FullEmpNum", "CertYear", "PplNumPerWk", "PplNumPerShift") #"PC1", "PC2", "PC3", "PC4", "PC5"
catVar = c("PartEmp", "Loc", "ParlorFluSys", "Glove", "HouseStyle")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_noPasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/MSC/NumVars_MSC_noPasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_noPasture_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/MSC/CatVars_MSC_noPasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")