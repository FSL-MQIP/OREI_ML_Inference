set.seed(1)
MSC_split= initial_split(MSC_newFarm, prop = 501/502, strata = logConc)
...
MSC_fit = MSC_wf %>% fit(data = MSC_newFarm)
...
MSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = MSC_newFarm,
  y = MSC_newFarm$logConc,
  verbose = F
)


numVar = c("CowNum", "CertYear", "PastureTime", "NonFamEmpNum", "SPCvar", "PplNumPerWk") 
catVar = c("DryMatPercent", "ParlorHose", "GloveFreq", "HouseStyle", "ClipFlame", "CornSilage",
           "Parlor")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_newFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/MSC/NumVars_MSC_newFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_newFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/MSC/CatVars_MSC_newFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")