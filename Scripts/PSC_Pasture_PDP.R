set.seed(1)
PSC_split= initial_split(PSC_Pasture, prop = 501/502, strata = Pres)
PSC_train = training(PSC_split)
PSC_test = testing(PSC_split)

PSC_recipe = recipe(Pres ~ ., data = PSC_Pasture) %>% 
...
PSC_fit = PSC_wf %>% fit(data = PSC_Pasture)
...
numVar = c("NonFamEmpNum",  "CowNum") #PC3
catVar = c("CowMilkLoc", "NonFamEmp", "ParlorHose", "TowelType", "ClosedHerd", "CornSilage",
           "Haylage", "ClipFlame", "Parlor", "ParlorManScrap", "Bed", "CowWaitMilk",
           "PreDipType", "TowelDeter")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_Pasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/PSC/NumVars_PSC_Pasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_Pasture_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/PSC/CatVars_PSC_Pasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")