set.seed(1)
BAB_split= initial_split(BAB_noPasture, prop = 501/502, strata = logConc)
BAB_train = training(BAB_split)
BAB_test = testing(BAB_split)

BAB_recipe = recipe(logConc ~ ., data = BAB_noPasture) %>% 
...
BAB_fit = BAB_wf %>% fit(data = BAB_noPasture)
...
BAB_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = BAB_noPasture,
  y = BAB_noPasture$logConc,
  verbose = F
)


numVar = c("CertYear", "FullEmpNum", "CowNum", "PplNumPerWk", "NonFamEmpNum", "StockDen",
           "PplNumPerShift", "StockDen")
catVar = c("PreDipType", "TowelType", "HoldFluSys", "Bed", "ClipFlame", "TowelDeter",
           "ParlorFluSys", "ClosedHerd")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_noPasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/BAB/NumVars_BAB_noPasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_noPasture_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/BAB/CatVars_BAB_noPasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")