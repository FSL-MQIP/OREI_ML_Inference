set.seed(1)
PSC_split= initial_split(PSC_newFarm, prop = 501/502, strata = Pres)
...
PSC_fit = PSC_wf %>% fit(data = PSC_newFarm)
...
numVar = c("PplNumPerShift", "CowNum", "PplNumPerWk", "CertYear", "FullEmpNum", "SPCvar") 
catVar = c("Bed", "HoldManScrap", "Glove", "GloveFreq", "CornSilage", "CowMilkLoc", "PreDipType")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_newFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/PSC/NumVars_PSC_newFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_newFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/PSC/CatVars_PSC_newFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")