set.seed(1)
TSC_split= initial_split(TSC_oldFarm, prop = 501/502, strata = logConc)
...
TSC_fit = TSC_wf %>% fit(data = TSC_oldFarm)
...
TSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = TSC_oldFarm,
  y = TSC_oldFarm$logConc,
  verbose = F
)


numVar = c("CowNum", "FullEmpNum", "SPCvar", "PplNumPerWk", "CertYear", "NonFamEmpNum",
           "PplNumPerShift")
catVar = c("PartEmp", "CowWaitMilk", "ClipFlame", "CowHoldClean", "HouseStyle", "CornSilage",
           "HoldManScrap", "CowMilkLoc")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_oldFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/TSC/NumVars_TSC_oldFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_oldFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/TSC/CatVars_TSC_oldFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")