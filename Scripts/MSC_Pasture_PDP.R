set.seed(1)
MSC_split= initial_split(MSC_Pasture, prop = 501/502, strata = logConc)
MSC_train = training(MSC_split)
MSC_test = testing(MSC_split)

MSC_recipe = recipe(logConc ~ ., data = MSC_Pasture) %>% 
...
MSC_fit = MSC_wf %>% fit(data = MSC_Pasture)
...
MSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = MSC_Pasture,
  y = MSC_Pasture$logConc,
  verbose = F
)


numVar = c("CowNum", "PplNumPerWk", "PastureTime", "CertYear", "FullEmpNum", "StockDen", "PplNumPerShift", "SPCvar") #PC1, PC5
catVar = c("PartEmp", "Loc", "CowMilkLoc", "ClipFlame", "BedAdd")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_Pasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/MSC/NumVars_MSC_Pasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_Pasture_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/MSC/CatVars_MSC_Pasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")