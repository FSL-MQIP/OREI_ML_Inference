set.seed(1)
PSC_split= initial_split(PSC_noPasture, prop = 501/502, strata = Pres)
PSC_train = training(PSC_split)
PSC_test = testing(PSC_split)

PSC_recipe = recipe(Pres ~ ., data = PSC_noPasture) %>% 
...
PSC_fit = PSC_wf %>% fit(data = PSC_noPasture)
...
PSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = PSC_noPasture,
  y = PSC_noPasture$Pres,
  verbose = F
)


numVar = c("PplNumPerShift", "CowNum", "StockDen", "CertYear", "PplNumPerWk", "NonFamEmpNum", "FullEmpNum") #PC3
catVar = c("Haylage", "Loc", "Glove", "ClipFlame", "CowHoldClean", "GloveFreq", "CowMilkLoc")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_noPasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/PSC/NumVars_PSC_noPasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/PSC/PSC_noPasture_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/PSC/CatVars_PSC_noPasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")