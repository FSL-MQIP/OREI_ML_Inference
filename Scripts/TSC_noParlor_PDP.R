set.seed(1)
TSC_split= initial_split(TSC_noParlor, prop = 501/502, strata = logConc)
TSC_train = training(TSC_split)
TSC_test = testing(TSC_split)

TSC_recipe = recipe(logConc ~ ., data = TSC_noParlor) %>% 
...
TSC_fit = TSC_wf %>% fit(data = TSC_noParlor)
...
TSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = TSC_noParlor,
  y = TSC_noParlor$logConc,
  verbose = F
)


numVar = c("CertYear", "SPCvar", "PplNumPerWk", "CowNum", "PplNumPerShift") #PC5
catVar = c("ClipFlame", "CornMeal", "BedAdd", "BedAddFreq", "DryHay", "CowParlorClean", "GloveFreq",
           "CornSilage", "ClosedHerd")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_noParlor_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/TSC/NumVars_TSC_noParlor.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/TSC/TSC_noParlor_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/TSC/CatVars_TSC_noParlor.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")