set.seed(1)
BAB_split= initial_split(BAB_Pasture, prop = 501/502, strata = logConc)
BAB_train = training(BAB_split)
BAB_test = testing(BAB_split)

BAB_recipe = recipe(logConc ~ ., data = BAB_Pasture) %>% 
...
BAB_fit = BAB_wf %>% fit(data = BAB_Pasture)
...
BAB_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = BAB_Pasture,
  y = BAB_Pasture$logConc,
  verbose = F
)


numVar = c("PastureTime", "StockDen", "CowNum", "CertYear", "SPCvar", "PplNumPerWk",
           "NonFamEmpNum", "FullEmpNum") #PC1, PC3
catVar = c("TowelDeter", "ClipFlame")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_Pasture_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/BAB/NumVars_BAB_Pasture.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_Pasture_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/BAB/CatVars_BAB_Pasture.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")