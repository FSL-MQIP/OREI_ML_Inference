set.seed(1)
BAB_split= initial_split(BAB_newFarm, prop = 501/502, strata = logConc)
...
BAB_fit = BAB_wf %>% fit(data = BAB_newFarm)
...
BAB_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = BAB_newFarm,
  y = BAB_newFarm$logConc,
  verbose = F
)


numVar = c("PastureTime", "CowNum", "CertYear", "SPCvar", "PplNumPerWk",
           "PplNumPerShift", "NonFamEmpNum") 
catVar = c("Parlor", "ParlorManScrap", "GloveFreq", "HoldFluSys", "NonFamEmp", "BedAddFreq")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_newFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/BAB/NumVars_BAB_newFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_newFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/BAB/CatVars_BAB_newFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")