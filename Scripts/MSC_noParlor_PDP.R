set.seed(1)
MSC_split= initial_split(MSC_noParlor, prop = 501/502, strata = logConc)
MSC_train = training(MSC_split)
MSC_test = testing(MSC_split)

MSC_recipe = recipe(logConc ~ ., data = MSC_noParlor) %>% 
...
MSC_fit = MSC_wf %>% fit(data = MSC_noParlor)
...
MSC_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = MSC_noParlor,
  y = MSC_noParlor$logConc,
  verbose = F
)


numVar = c("CertYear", "PastureTime", "StockDen", "PplNumPerWk", "CowNum", "SPCvar") #PC1
catVar = c("CowParlorClean", "Glove", "CornSilage", "ClipFlame", "GloveFreq", "ClosedHerd", "DryHay", "BedAddFreq")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_noParlor_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/MSC/NumVars_MSC_noParlor.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/MSC/MSC_noParlor_pdp_", catVar[i], ".tiff")
  ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/MSC/CatVars_MSC_noParlor.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")