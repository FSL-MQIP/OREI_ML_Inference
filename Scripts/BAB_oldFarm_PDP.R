set.seed(1)
BAB_split= initial_split(BAB_oldFarm, prop = 501/502, strata = logConc)
...
BAB_fit = BAB_wf %>% fit(data = BAB_oldFarm)
...
BAB_explainer = explain_tidymodels(
  final_model$.workflow[[1]],
  data = BAB_oldFarm,
  y = BAB_oldFarm$logConc,
  verbose = F
)


numVar = c("PastureTime", "StockDen", "CowNum", "PplNumPerWk", "NonFamEmpNum", "PplNumPerShift",
           "CertYear", "SPCvar", "FullEmpNum")
catVar = c("ClipFlame", "TowelDeter", "PreDipType", "Haylage")
...
for (i in 1:length(numVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_oldFarm_pdp_", numVar[i], ".tiff")
...
ggsave("Figure/Partial dependence plots/BAB/NumVars_BAB_oldFarm.pdf", numPlots_grid, 
...
for (i in 1:length(catVar)) {
  ...
  file_name = paste0("Figure/Partial dependence plots/BAB/BAB_oldFarm_pdp_", catVar[i], ".tiff")
  #ggsave(file_name, height = 4, width = 4, units = "in", dpi = "retina")
...
ggsave("Figure/Partial dependence plots/BAB/CatVars_BAB_oldFarm.pdf", catPlots_grid,
       height = 4, width = 8, units = "in", dpi = "retina")