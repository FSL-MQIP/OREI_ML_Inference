...
write.csv(FarmData, "FarmData.csv")
#skim(FarmData)
```
...
## spore count by type 
FarmData %>% 
  filter(Test %in% c("MSC", "TSC", "BAB")) %>% 
  ggplot(aes(x = log10(Conc), fill = Test)) +
  geom_histogram() +
  facet_wrap(vars(Test)) +
  theme_classic() +
  theme(legend.position = "none") +
  labs(x = expression(paste(log[10], "CFU/mL or ", log[10], "MPN/mL")),
       y = "Frequency")
ggsave("Manuscript/Fig1.tiff", height = 4, width = 8, units = "in", dpi = "retina")
```
...
MSCdata = FarmData %>% 
  ...
  select(-c(GreaterLess, Conc, FarmID, SampleTime, Sampling, University, Coop, Loc))
